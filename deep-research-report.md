...
| 検出工程 | 指摘・不良を発見した工程 | `外部仕様書`, `内部仕様書`, `コード`, `テスト仕様書`, `リリース後` (`後工程` は既存互換の別名) | 工程別検出件数、工程別レビュー効果、流出判定に使う |
...
| 用語 | 定義 |
|---|---|
| 工程順 | `外部仕様書 < 内部仕様書 < コード < テスト仕様書 < リリース後` (`後工程` は `リリース後` に正規化) |
...
| テスト仕様書 | 後工程 | いいえ | いいえ | `後工程` は `リリース後` の別名として扱う。テスト仕様書起因であり、コード以前の工程流出ではない |
...
```text
全体流出率 =
  リリース後で検出された不良件数（後工程表記はリリース後へ正規化）
  / 最終把握不良件数
